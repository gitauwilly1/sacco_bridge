...
from apps.users.serializers import (
    ...
    LoginHistorySerializer, SessionSerializer,
)
from apps.users.services import (
    AuthenticationService, TwoFactorService, GoogleAuthService
)
...
class RegistrationView(APIView):
    ...
    @extend_schema(
        ...
        description='Creates a new user account with email and phone verification.',
...
class TwoFactorSetupView(APIView):
    ...
    @extend_schema(
        ...
        description='Returns current 2FA status and QR code for setup.',
        responses={
            200: OpenApiExample(
                '2FA Setup Info',
                value={
        ...
        if user.two_factor_enabled:
            return Response({
                ...
                'data': {
                    ...
                    'message': _('Two-factor authentication is enabled.'),
                }
            })
    ...
    @extend_schema(
        ...
        description='Enable 2FA after verifying the setup with a TOTP code.',
    ...
    def post(self, request):
        ...
        if not serializer.validated_data.get('enable'):
            return Response({
                ...
                'error': {
                    ...
                    'message': _('Set enable=true to activate 2FA.'),
                }
        ...
        try:
            TwoFactorService.enable_two_factor(request.user, secret, totp_code)
        ...
        return Response({
            ...
            'data': {
                'two_factor_enabled': True,
                'message': _('Two-factor authentication has been enabled.'),
            },
    ...
    def delete(self, request):
        ...
        if not totp_code:
            return Response({
                ...
                'error': {
                    'code': 'validation_error',
                    'message': _('TOTP code is required to disable 2FA.'),
                }
            }, status=status.HTTP_400_BAD_REQUEST)
...
class EmailVerificationView(APIView):
    ...
    @extend_schema(
        ...
        description='Verify email using the OTP sent during registration.',
...
class PhoneVerificationView(APIView):
    ...
    @extend_schema(
        ...
        description='Verify phone number using the OTP sent via SMS.',
...
class ResendVerificationView(APIView):
    ...
    @extend_schema(
        ...
        description='Request a new verification code by email or SMS.',
    ...
    def post(self, request):
        ...
        if method == 'email':
            AuthenticationService.send_verification_email(user)
            message = _('Verification code sent to %(email)s.') % {'email': user.email}
        else:
            AuthenticationService.send_verification_sms(user)
            message = _('Verification code sent to %(phone)s.') % {'phone': user.phone_number}
...
class UserProfileDetailView(APIView):
    ...
    @extend_schema(
        ...
        description='Retrieve extended profile with KYC and address information.',
    )
    def get(self, request):
    ...
    @extend_schema(
        ...
        description='Update extended profile fields.',
    )
    def patch(self, request):
        profile = request.user.profile
...
class ActiveSessionsView(APIView):
    ...
    @extend_schema(
        ...
        description='End a specific session (logout from another device).',
    )
