...
class SpecialCharacterValidator:
    ...
    def get_help_text(self):
        return _(
            'Your password must contain at least one special character: %(chars)s.'
        ) % {'chars': self.special_characters}
...
def validate_kenyan_phone_number(value):
    ...
    if not re.match(pattern, cleaned):
        raise ValidationError(
            _('%(value)s is not a valid Kenyan phone number.'),
            params={'value': value},
        )
...
def validate_positive_amount(value):
    ...
    if value > Decimal('100000000.00'):
        raise ValidationError(
            _('Amount cannot exceed KSh 100,000,000.'),
        )
...
def validate_share_quantity(value):
    ...
    if value <= Decimal('0.0000'):
        raise ValidationError(
            _('Share quantity must be greater than zero.'),
        )
    if value > Decimal('10000000.0000'):
        raise ValidationError(
            _('Share quantity cannot exceed 10,000,000.'),
        )
...
def validate_id_number(value):
    ...
    if not re.match(pattern, str(value)):
        raise ValidationError(
            _('%(value)s is not a valid Kenyan ID number.'),
            params={'value': value},
        )